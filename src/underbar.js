(function() {
  ...
  _.filter = function(collection, test, f) {
    var res=[];
    var falseRes=[];

    _.each(collection,function(item) {
      test(item) ? res.push(item) : falseRes.push(item);
    });

    return f === 1 ? falseRes : res;
  };
  ...
  _.reject = function(collection, test) {
    ...
    return _.filter(collection,test,1);
  };
  ...
  _.uniq = function(array) {
    ...
    _.each(array,function(item) {
      _.indexOf(res,item) === -1 ? res.push(item) : 1;
    });
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    if(collection.length === 0) {
     ...
     return _.reduce(collection,iterator,iterator(accumulator,collection.shift()));
    }
  };
  ...
  _.every = function(collection, iterator, f) {
    // TIP: Try re-using reduce() here.
    f === undefined ? f=true : f=false;
  ...
  _.extend = function(obj) {
    ...
    _.each(_.last(Array.prototype.slice.apply(arguments),arguments.length-1),function(ob,index) {
  ...
  _.defaults = function(obj) {
    ...
    _.each(_.last(Array.prototype.slice.apply(arguments),arguments.length-1),function(ob,index) {
      _.each(ob,function(value,prop) {
        obj[prop] === undefined ? obj[prop] = value : 1;
      });
    });
  ...
  _.shuffle = function(array) {
    var arrCopy = Array.prototype.slice.apply(array);
    var newArr = [];
    var random = 0;
    ...
    while(newArr.length < length) {
      ...
      newArr.push(arrCopy.splice(random,1)[0]);
    };
  ...
  _.invoke = function(collection, functionOrKey, args) {
    if(typeof functionOrKey !== "function") {
      functionOrKey = Array.prototype[functionOrKey] || String.prototype[functionOrKey];
    }
